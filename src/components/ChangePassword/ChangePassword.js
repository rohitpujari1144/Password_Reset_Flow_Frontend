...
function ChangePassword() {
    ...
    function changePasswordClick() {
        ...
        if (passwordError.innerText === "" && confirmPasswordError.innerText === '' && passwordResetCodeError.innerText === '') {
            axios.get(`https://password-reset-flow-backend-p97p.onrender.com/getUser/${userEmail}`)
                .then((response) => {
                    if (response.data.data[0].randomString === passwordResetCode.value) {
                        let newPassword = {
                            password: password.value,
                            randomString:''
                        }
                        axios.put(`https://password-reset-flow-backend-p97p.onrender.com/changePassword/${userEmail}`, newPassword)
                            .then((res) => {
                                setOpen(true)
                                setTimeout(() => {
