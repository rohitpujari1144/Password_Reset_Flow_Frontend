...
function ForgotPassword() {
  ...
  const [verificationRandomString, setVerificationRandomString] = useState("");

  async function sendEmail(userEmail, verificationString) {
    let emailInfo = {
      verificationRandomString: verificationRandomString,
      userEmail: userEmail
    }
    await axios.post('https://password-reset-flow-backend-p97p.onrender.com/sendEmail', emailInfo)
      .then((res) => {
        ...
        axios.put(`https://password-reset-flow-backend-p97p.onrender.com/storeRandomString/${userEmail}`, randomString)
          .then((response) => {
            setVerificationRandomString(verificationString)
            sessionStorage.setItem('verificationString', verificationString)
  ...
  async function randomStringGenerator() {
    ...
    const verificationString = await generateString(10);
    sendEmail(username.value, verificationString)
  }
